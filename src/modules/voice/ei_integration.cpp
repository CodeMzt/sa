/**
 * @file ei_integration.cpp
 * @brief Edge Impulse 模型推理集成实现（音频缓冲管理与推理调度）
 * @date 2026-02-11
 * @author Ma Ziteng
 */
...
static int get_signal_data(size_t offset, size_t length, float *out_ptr) {
    int32_t read_start = (int32_t)buf_write_index - EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE + (int32_t)offset;
    if (read_start < 0) read_start += AUDIO_BUFFER_SIZE;

    size_t dist_to_end = AUDIO_BUFFER_SIZE - (size_t)read_start;
    if (length <= dist_to_end) {
        arm_q15_to_float(&audio_buffer[read_start], out_ptr, (uint32_t)length);
    } else {
        arm_q15_to_float(&audio_buffer[read_start], out_ptr, (uint32_t)dist_to_end);
        size_t remaining = length - dist_to_end;
        arm_q15_to_float(&audio_buffer[0], &out_ptr[dist_to_end], (uint32_t)remaining);
    }
    return EIDSP_OK;
}
...
int ei_run_inference(void) {
    if (!buffer_full_enough && buf_write_index < EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE) return -1;
...
const char* ei_get_label_name(int class_idx) {
    if (class_idx >= 0 && class_idx < EI_CLASSIFIER_LABEL_COUNT) return ei_classifier_inferencing_categories[class_idx];
    return "Unknown";
}
