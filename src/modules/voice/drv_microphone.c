...
static void signal_audio_ready(void) {
    for(uint16_t i = 0; i < BLOCK_SIZE; i++) {
        ...
        audio_data = (int16_t)__SSAT(audio_data * MIC_GAIN, 16);
        audio_read_buffer[read_buffer_index][i] = (int32_t)audio_data;
    }
#ifndef TEST
    ei_feed_audio(audio_read_buffer[read_buffer_index], BLOCK_SIZE);
#endif
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    xSemaphoreGiveFromISR(audio_semaphore, &xHigherPriorityTaskWoken);
