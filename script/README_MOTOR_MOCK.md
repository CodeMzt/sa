...
| `udp_reliability_receiver.py` | 可靠性测试接收端，统计延迟、抖动和估算丢包 |
...
```bash
python script/mock_motor_sender.py
```
...
编辑 [mock_motor_sender.py](e:/Documents/e2_studio/sa/script/mock_motor_sender.py) 中的：
...
编辑 [mock_motor_sender.py](e:/Documents/e2_studio/sa/script/mock_motor_sender.py) 中的 `generate_motor_states()`。
