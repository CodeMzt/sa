...
#if 1
static StaticTask_t log_task_memory;
#if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t log_task_stack[0x800] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t log_task_stack[0x800] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.log_task") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
TaskHandle_t log_task;
void log_task_create(void);
static void log_task_func(void *pvParameters);
...
sci_uart_instance_ctrl_t g_uart_log_ctrl;

baud_setting_t g_uart_log_baud_setting =
        {
...
const sci_uart_extended_cfg_t g_uart_log_cfg_extend =
{ .clock = SCI_UART_CLOCK_INT, .rx_edge_start = SCI_UART_START_BIT_FALLING_EDGE, .noise_cancel =
          SCI_UART_NOISE_CANCELLATION_DISABLE,
  .rx_fifo_trigger = SCI_UART_RX_FIFO_TRIGGER_MAX, .p_baud_setting = &g_uart_log_baud_setting, .flow_control =
...
/* Instance structure to use this module. */
const uart_instance_t g_uart_log =
{ .p_ctrl = &g_uart_log_ctrl, .p_cfg = &g_uart_log_cfg, .p_api = &g_uart_on_sci };
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t log_task_parameters =
{ .p_context = (void*) NULL, };
...
void log_task_create(void)
{
...
#else
                    BaseType_t log_task_create_err = xTaskCreate(
                    #endif
                                  log_task_func,
                                  (const char*) "log_task", 0x800 / 4, // In words, not bytes
                                  (void*) &log_task_parameters, //pvParameters
                                  1,
#if 1
                                  (StackType_t*) &log_task_stack,
                                  (StaticTask_t*) &log_task_memory
#else
                        & log_task
                        #endif
                                  );
    ...
    if (NULL == log_task)
    {
        rtos_startup_err_callback (log_task, 0);
    }
#else
                    if (pdPASS != log_task_create_err)
                    {
                        rtos_startup_err_callback(log_task, 0);
                    }
                    #endif
