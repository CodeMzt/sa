...
void net_connect_create(void);
static void net_connect_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t net_connect_parameters =
{ .p_context = (void*) NULL, };
...
void net_connect_create(void)
{
    ...
    if (NULL == net_connect)
    {
        rtos_startup_err_callback (net_connect, 0);
    }
#else
                    if (pdPASS != net_connect_create_err)
                    {
                        rtos_startup_err_callback(net_connect, 0);
                    }
                    #endif
...
static void net_connect_func(void *pvParameters)
{
    ...
    net_connect_entry (pvParameters);
}
