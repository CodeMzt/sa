...
#if 1
static StaticTask_t wifi_debug_memory;
#if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t wifi_debug_stack[0x8000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t wifi_debug_stack[0x8000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.wifi_debug") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
TaskHandle_t wifi_debug;
void wifi_debug_create(void);
static void wifi_debug_func(void *pvParameters);
...
sci_uart_instance_ctrl_t g_uart_wifi_ctrl;

baud_setting_t g_uart_wifi_baud_setting =
        {
...
const sci_uart_extended_cfg_t g_uart_wifi_cfg_extend =
{ .clock = SCI_UART_CLOCK_INT, .rx_edge_start = SCI_UART_START_BIT_FALLING_EDGE, .noise_cancel =
          SCI_UART_NOISE_CANCELLATION_DISABLE,
  .rx_fifo_trigger = SCI_UART_RX_FIFO_TRIGGER_MAX, .p_baud_setting = &g_uart_wifi_baud_setting, .flow_control =
...
/* Instance structure to use this module. */
const uart_instance_t g_uart_wifi =
{ .p_ctrl = &g_uart_wifi_ctrl, .p_cfg = &g_uart_wifi_cfg, .p_api = &g_uart_on_sci };
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t wifi_debug_parameters =
{ .p_context = (void*) NULL, };
...
void wifi_debug_create(void)
{
...
#if 1
    wifi_debug = xTaskCreateStatic (
#else
                    BaseType_t wifi_debug_create_err = xTaskCreate(
                    #endif
                                    wifi_debug_func,
                                    (const char*) "wifi_debug", 0x8000 / 4, // In words, not bytes
                                    (void*) &wifi_debug_parameters, //pvParameters
                                    2,
#if 1
                                    (StackType_t*) &wifi_debug_stack,
                                    (StaticTask_t*) &wifi_debug_memory
#else
                        & wifi_debug
                        #endif
                                    );
    ...
    if (NULL == wifi_debug)
    {
        rtos_startup_err_callback (wifi_debug, 0);
    }
#else
                    if (pdPASS != wifi_debug_create_err)
                    {
                        rtos_startup_err_callback(wifi_debug, 0);
                    }
                    #endif
